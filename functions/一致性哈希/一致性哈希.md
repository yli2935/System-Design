...
```
受影响范围：(s3, s4]
需要从 s3 迁移到 s4 的键：位于 (s3, s4] 区间的所有键
```

#### 移除服务器

```
受影响范围：[逆时针的前一个服务器, 被移除的服务器]
```
...
def create_virtual_nodes(server_name, num_virtual_nodes):
    ...
    for i in range(num_virtual_nodes):
        virtual_node_name = f"{server_name}_#{i}"
        hash_value = hash_function(virtual_node_name)
        virtual_nodes.append((hash_value, server_name))
...
class ConsistentHash:
    ...
    def __init__(self, num_virtual_nodes: int = 150):
        ...
        self.servers: Dict[str, int] = {}  # {server: weight}
        self.load: Dict[str, int] = {}     # {server: current_load}
    ...
    def add_server(self, server: str, weight: int = 1) -> None:
        ...
        # 根据权重创建虚拟节点
        num_vnodes = self.num_virtual_nodes * weight
    ...
    def get_load_stats(self) -> Dict[str, Dict]:
        """获取负载统计"""
        total_load = sum(self.load.values())
        avg_load = total_load / len(self.servers) if self.servers else 0

        stats = {}
        for server in self.servers:
            current_load = self.load[server]
            stats[server] = {
                'load': current_load,
                'percentage': (current_load / total_load * 100) if total_load > 0 else 0,
                'deviation': ((current_load - avg_load) / avg_load * 100) if avg_load > 0 else 0
            }
...
def demo():
    ...
    # 添加服务器（带权重）
    ch.add_server("server-01", weight=2)  # 高性能
    ch.add_server("server-02", weight=1)  # 普通
    ch.add_server("server-03", weight=1)  # 普通
    ...
    # 查看负载统计
    print("\n负载统计:")
    stats = ch.get_load_stats()
    for server, info in stats.items():
        print(f"{server}: {info['load']} 请求 ({info['percentage']:.2f}%), "
              f"偏差: {info['deviation']:.2f}%")

    # 添加新服务器
    print("\n添加新服务器 server-04...")
    ch.add_server("server-04", weight=1)

    # 移除服务器
    print("\n移除服务器 server-02...")
    ch.remove_server("server-02")

    # 获取键的多个副本服务器
    replicas = ch.get_servers_for_key("important-data", count=3)
    print(f"\nimportant-data 的副本服务器: {replicas}")
